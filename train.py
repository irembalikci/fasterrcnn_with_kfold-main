...
def main(args):
    ...
    for fold, (train_indices, valid_indices) in enumerate(kf.split(train_dataset)):
        ...
        for epoch in range(start_epochs, NUM_EPOCHS):
            ...
            # Store losses and mAPs for this fold
            fold_losses.append(train_loss_hist.value)
            fold_maps.append(stats[0])  # Assuming stats[0] holds the mAP value
        
    # After the K-fold cross-validation, calculate the average loss and mAP
    average_loss = np.mean(fold_losses)
    average_map = np.mean(fold_maps)

    print(f"Average Loss across {k} folds: {average_loss}")
    print(f"Average mAP across {k} folds: {average_map}")
